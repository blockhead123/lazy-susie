...
class DataModel EXTENDS query{
	...
	function get_records($filterstr = ''){
	...
	function get_record($id='', $filterstr=''){
		if(strlen($this->sql)>0){
			$sql = $this->sql;
		}else{
			$sql = 'SELECT * FROM '.$this->tblname;
			if($id!=''){
				$sql.= ' WHERE '.$this->primekey.'=\''.$id.'\'';
			}elseif($filterstr!=''){
				$sql.= ' WHERE '.$filterstr;
			}
		}
	...
	function generate_statement($statement_type, $postvars, $fieldprefix){
		$this->get_field_definitions($this->tblname);
		$statement ='';
		$errorval=false;
		$post=$postvars;
		...
		$sql=($statement_type=='add')?'INSERT INTO '.$this->tblname.' SET '.$statement:'UPDATE '.$this->tblname.' SET '.$statement;
		if($errorval){
			return false;
		}else{
			return $sql;
